...
//    private Cipher mCipher;
    private ThreadLocal<SoftReference<Cipher>> mCipher = new ThreadLocal<>();
...
//            Arrays.fill(rawEncryptionKey, secureByte);
    ...
    private Cipher getCipher() {
        final SoftReference<Cipher> reference = mCipher.get();
        return Optional.ofNullable(reference).map(SoftReference::get).orElseGet(() -> {
            Cipher cipher = createCipher();
            mCipher.set(new SoftReference<>(cipher));
            return cipher;
        });
    }
